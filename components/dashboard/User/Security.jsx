...
const Security = ({token, user}) => {
  ...
  return (
    <div className="shadow-lg shadow-indigo-600 p-2 rounded h-screen">
        <div className="text-lg  flex items-center"><div className='w-2 h-2 mr-2 bg-gray-600 rounded-full'></div>Change your password</div>
        <div className='mt-3 ml-2'>
            <form onSubmit={handleSubmit}>
            <div className="flex flex-col">
                <label className="text-sm mb-1 ">Current Password</label>
                <input type="password"
                    className="px-2 text-sm outline-indigo-600 focus:outline text-white bg-gray-600 w-1/2 h-10 rounded"
                    value={currentPassword}
                    onChange={(e)=>setCurrentPassword(e.target.value)} />
            </div>
            <div className="flex flex-col mt-2">
                <label className="pl-1 text-sm mb-1 ">New Password</label>
                <input type="password"
                    className="px-2 text-sm outline-indigo-600 focus:outline text-white bg-gray-600 w-1/2 h-10 rounded"
                    value={newPassword}
